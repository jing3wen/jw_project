<template>
  <el-card >
    ...
    <el-form class="query-form pl-10" :model="queryForm">
      <el-input style="width: 200px" suffix-icon="el-icon-search" placeholder="请输入标签名"
                v-model="queryForm.tagName" @keyup.enter.native="search"></el-input>
...
export default {
  ...
  methods: {
      ...
      this.$refs['dialogForm'].validate((valid) => {
        if (valid) {
          let url = this.dialogForm.categoryId == null ?
              '/api/blogTag/admin/addBlogTag' : '/api/blogTag/admin/updateBlogTag'
    ...
    handleSelectionChange(selectedRows) {
      this.selectedRowIds = selectedRows.map(item => item.tagId)
    ...
    deleteRows(id){
      ...
      this.$confirm('确定要删除此'+ids.length+'条数据吗?', '系统提示', {type : 'warning'}).then(() => {
        this.request.delete('/api/blogTag/admin/deleteBatch', {data:ids}).then(res =>{
          if(res.code === 200){
