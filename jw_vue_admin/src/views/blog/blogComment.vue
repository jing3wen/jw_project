<template>
  <el-card >
    ...
    <el-table :data="tableData"
      ...
      <el-table-column prop="commentContent" label="评论内容" width="300px" :show-overflow-tooltip="true" align="center"></el-table-column>
      ...
      <el-table-column prop="createTime" label="评论时间" width="180px" align="center"></el-table-column>
      <el-table-column prop="operate" label="操作" fixed="right" width="200px" align="center">
        <template slot-scope="scope">
          <el-button v-if="scope.row.commentCheck==='0'"
                     type="text"
                     class="el-icon-edit pl-10"
                     @click="updateCommentCheckBatch(scope.row.commentId)">审核</el-button>
...
export default {
  ...
  methods: {
    getPageList(){
      this.request.post('/api/blogComment/admin/getPageList',this.queryForm).then(res =>{
        if(res.code === 200){
    ...
    //审核评论
    updateCommentCheckBatch(id){
      let ids = []
      if(id == null)
        ids = this.selectedRowIds
      else
        ids = [id]
      this.$confirm('确定要审核此'+ids.length+'条评论吗?', '系统提示', {type : 'warning'}).then(() => {
        this.request.post('/api/blogComment/admin/updateCommentCheckBatch',ids).then(res =>{
          if(res.code === 200) {
