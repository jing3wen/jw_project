...
class Retinaface(object):
    ...
    """
    将image_paths(人脸数据库)目录下的所有图片通过retinaface提取出人脸边界框
    将提取的人脸输入到facenet得到128的特征向量
    将所有图片的128维的人脸特征向量存入到 mobilenet_face_encoding.npy
    将所有图片标签存入到 mobilenet_names.npy
    """
    ...
    def detect_image(self, image):
        """
        recognition_face_list用来存储检测图片通过facenet识别出来的人脸
        num_face_Image表示图片检测出的人脸数
        """
        ...
        with torch.no_grad():
            ...
            # train_time_start = time.time()    # out = net(image)
            loc, conf, landms = self.net(image)
            # torch.cuda.synchronize()
            # train_time_end = time.time()
            # print(f"**检测一张图片时间为 {(train_time_end-train_time_start)}ms")
            ...
            """
            if len(boxes_conf_landms) <= 0:
                return old_image, recognition_face_list
        ...
        for boxes_conf_landm in boxes_conf_landms:
            ...
            with torch.no_grad():
                ...
                # torch.cuda.synchronize()
                # train_time_end = time.time()
                # print(f"**提取一张人脸特征时间为 {(train_time_end - train_time_start)}ms")
                face_encodings.append(face_encoding)
        # -----------------------------------------------#
        #   Facenet编码部分-结束
        # -----------------------------------------------#
        ...
        for i, b in enumerate(boxes_conf_landms):
            ...
            """
            # cv2只显示英文代码
            # 一张人脸情况下只显示英文时fps=23,显示中文时fps=21
        ...
        for name in face_names:
            if name != 'Unknown':
                recognition_face_list.append(name)
        return old_image, recognition_face_list
