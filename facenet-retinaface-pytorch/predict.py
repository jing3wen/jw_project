...
def model_predict(mode="video", video_path=0, video_save_path="", video_fps=25.0, test_interval=100,
    ...
    student_course_score_dict = {}  # 记录学生上课得分情况
    student_total = 0  # 识别出的最多人数，用于计算抬头率，主要用于视频检测
    ...
    if mode == "predict":
        ...
        while True:
            ...
            image = cv2.imread(img)  # image_read
            if image is None:
                print('Open Error! Try again!')
                continue
            else:
                image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # Red Green Blue
                r_image, recognition_face_list = retinaface.detect_image(image)
...
if __name__ == "__main__":
    model_predict(mode='predict', video_path='img/hezhao.png')
