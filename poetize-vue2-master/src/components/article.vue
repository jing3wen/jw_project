    ...
    <div style="background: var(--background);">
      <div class="article-container my-animation-slide-bottom">
        ...
        <div class="article-sort">
          <span @click="$router.push({path: '/sort', query: {sortId: article.categoryId}})">
          ...
          <span v-for="(tag, index) in article.tagList" :key="index"
           @click="$router.push({path: '/sort', query: {labelId: tag.tagId}})">
            <svg viewBox="0 0 1024 1024" width="15" height="15" style="vertical-align: -2px;">
              <path
          ...
          <!-- <span @click="$router.push({path: '/sort', query: {sortId: article.sortId, labelId: article.labelId}})">{{ " ▶ "+ article.label.labelName}}</span> -->
        </div>
        <!-- 作者信息 -->
        <blockquote>
          <div>
        ...
        <div class="myCenter" id="article-like">
          <i class="el-icon-thumb article-like-icon" :class="{'article-like': article.likedCounts}"></i>
        </div>

        <!-- 评论 -->
        <div v-if="article.commentAllowed === '1'">
          <comment :type="'article'" :source="article.articleId" :userId="article.userId"></comment>
        </div>
        <div v-else style="text-align: center; font-size: 25px; font-weight: bolder;">
          <span>
            作者已关闭评论区
          </span>
        </div>
